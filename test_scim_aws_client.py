...
def test_process_event(setup_connections):
    # Since we individually test all the operations, for process_event testing we are just
    # going to create a few events with a test user that we created in KC and verify
    # that the method is properly processing the events
    ...
    # To test process_event method, we are going to have to first create a user and then construct
    # an event that would be generated by scim-event-listener to pass to the method
    userobj = {}
    ...
    event = {}
    event['opType'] = "CREATE"
    event['resourcePath'] = f"users/{userobj[0]['id']}"
    ...
    event['representation'] = json.dumps(representation)
    result = scim_client_kc_aws.process_event(event)
    # Should not be provisioned since no attribute was set
    assert result.find("not created in scim") > 0

    # Test updating with aws enabled attribute
    event['opType'] == "UPDATE"
    attributes = {"awsenabled":["true"]}
    ...
    # To properly test deletion we need to get the users AWS ID
